...
test("도달권부터 대중교통 평가까지 진행률과 후보 순위를 만든다", async () => {
  ...
  const purposes = { isochrone: [], hub: [], transit: [] };
  const providers = {
    odsayIsochrone: async ({ purpose }) => {
      purposes.isochrone.push(purpose);
      return ({
      data: area,
      record: { provider: "ODSAY", status: 200, durationMs: 1, bytes: 10, attempts: 1 },
      });
    },
    kakaoKeyword: async ({ purpose }) => {
      purposes.hub.push(purpose);
      return ({
    ...
    tmapTransit: async ({ start, purpose }) => {
      purposes.transit.push(purpose);
      return ({
  ...
  assert.deepEqual(purposes.isochrone, ["TRANSIT_ISOCHRONE", "TRANSIT_ISOCHRONE"]);
  assert.deepEqual(purposes.hub, [
    "HUB_KEYWORD_SEARCH",
    "HUB_KEYWORD_SEARCH",
    "HUB_KEYWORD_SEARCH",
    "HUB_KEYWORD_SEARCH",
  ]);
  assert.deepEqual(purposes.transit, [
