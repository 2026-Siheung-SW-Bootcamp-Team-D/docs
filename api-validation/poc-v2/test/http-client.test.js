...
test("JSON이 아닌 응답은 분류 가능한 상류 계약 오류로 거부한다", async () => {
  const client = createHttpClient({
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      error.classification === "UPSTREAM_BAD_RESPONSE" &&
      error.status === 200 &&
      error.publicMessage === "외부 서비스 요청을 처리하지 못했습니다." &&
      !/bad gateway|html/i.test(error.message)
  );
});
