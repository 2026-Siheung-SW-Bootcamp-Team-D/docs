...
| Production | `main` | `https://api.yeondang.com` |
...
- Production 도메인은 `yeondang.com`과 `www.yeondang.com`을 사용한다. `VITE_API_BASE_URL`을 바꾼 뒤에는 Production 재배포가 필요하다.
...
> 외부 API 호출에는 Spring Framework 7의 **`RestClient`** 를 사용한다. `spring-boot-starter-webmvc`에 이미 포함되어 있어 **WebFlux 의존성을 추가하지 않는다.** 외부 호출은 단일 실행기에서 동시성 1로 순차 처리하므로 리액티브 스택이 필요하지 않다.
...
1. Cloud DNS 네임서버 위임 후 `api.yeondang.com A 34.50.11.59`를 등록한다.
2. Vercel에 `yeondang.com` 및 `www.yeondang.com`을 연결하고, Vercel 화면이 안내한 apex A 레코드와 www CNAME을 Cloud DNS에 등록한다.
3. Traefik이 `api.yeondang.com` Let's Encrypt 인증서를 발급했는지 확인한다.
4. Kakao Developers 허용 도메인에 `https://yeondang.com` 및 `https://www.yeondang.com`을 추가한다.
5. Spring CORS 허용 목록에 두 운영 프론트 URL을 추가하고 승인된 Vercel Preview 패턴은 유지한다.
6. **ODsay에 고정 IP 등록** — 이걸 빠뜨리면 지역 찾기가 통째로 실패한다
...
| 환경 | 방식 |
|---|---|
| Local | 저장소 루트의 `.env` (git 무시). `.env.example`에는 **변수 이름만** 둔다 |
| Production | VM 시작 스크립트가 서비스 계정 권한으로 Secret Manager에서 값을 읽어 `0600` 권한 env 파일로 쓰고, Docker Compose의 `env_file`로 컨테이너에 주입한다 |

애플리케이션은 두 환경 모두 **환경변수만 읽는다**(`${DB_PASSWORD}` 형태). 프로필별 코드 분기를 만들지 않는다.

> 앱이 값을 쓰려면 실행 중인 프로세스 환경에는 평문이 존재할 수밖에 없다. 따라서 보안 기준은 "프로세스 안에 평문이 없을 것"이 아니라 **"git 저장소·컨테이너 이미지·로그·오류 응답에 남지 않을 것"** 이다.
...
- **참여 토큰 secret만** 원문이 아니라 HMAC 값으로 저장한다. 초대 코드와 공개 토큰은 호스트 화면·공유 링크에서 원문을 다시 보여줘야 하므로 원문으로 저장하고, 만료·보드 상태로 통제하며 로그에서 마스킹한다. (API명세서 2.1절)
...
backend/src/main/resources/
  ...
  application-prod.yml     # 값이 아닌 환경변수 참조. 값의 출처는 Secret Manager (7절 주입 경로)
...
| 8 | Secret Manager 연동 | 비밀이 git·이미지·로그에 없음 (`0600` env 파일에는 존재. 7절 주입 경로) |
