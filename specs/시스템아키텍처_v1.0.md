...
6. 오래 걸리는 계산(지역 찾기, 출발 안내)은 **작업 테이블에 저장**하고 즉시 응답한다.
...
지역 찾기와 출발 안내는 외부 API 때문에 수 초 이상 걸린다. HTTP 요청 안에서 기다리지 않는다.
...
1. `@Scheduled`가 짧은 주기로 실행 가능한 작업을 조회한다.
2. `SELECT ... FOR UPDATE SKIP LOCKED`로 **한 작업만** 선점한다. (PostgreSQL 지원)
3. `RUNNING`으로 바꾸고 **즉시 트랜잭션을 끝낸다.**
...
7. 재시도 대상은 `RETRY_WAIT` 상태와 `nextRetryAt`을 저장한다.
8. 서버 재시작 후 오래된 `RUNNING` 작업은 `QUEUED`로 복구한다.
9. 같은 보드의 지역 찾기 작업이 동시에 두 개 뜨지 않도록 유일 제약이나 선점 검사를 둔다.
...
작업 상태별 개수(QUEUED·RUNNING·RETRY_WAIT·FAILED) / 외부 API 제공자별 호출 수·429·성공률 / VM 자원.
...
- 기능은 `기능명세서_v1.3.md`, API 계약은 `API명세서_v1.1.md`가 기준이다.
...
외부 API 작업은 작업 테이블에 저장하고, 짧은 트랜잭션으로 SKIP LOCKED 선점한 뒤
트랜잭션 밖에서 호출한다. 429 재시도·재시작 복구·중복 실행 방지를 구현한다.
